...
// --- anti-spam / security helpers (injected) ---
const ORIGIN = process.env.ORIGIN || 'https://karmanovy.ru';
function isSameOrigin(req){
  const o = req.headers.origin || '';
  const r = req.headers.referer || '';
  return o === ORIGIN || r.startsWith(ORIGIN);
}

const RL_MIN = new Map();  // 1 / minute
const RL_HR  = new Map();  // 5 / hour
function rateOk(ip){
  const now = Date.now();
  const m = (RL_MIN.get(ip)||[]).filter(t => now - t < 60*1000); m.push(now); RL_MIN.set(ip,m);
  if (m.length > 1) return false;
  const h = (RL_HR.get(ip)||[]).filter(t => now - t < 3600*1000); h.push(now); RL_HR.set(ip,h);
  return h.length <= 5;
}

const BAD = /(http|www\.|@|money|crypto|account|market)/i;
function validBody(b){
  if (!b || typeof b !== 'object') return false;
  if (!b.name || b.name.length < 2 || b.name.length > 80) return false;
  if (BAD.test(b.name) || BAD.test(b.notes||'')) return false;
  if (b.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(b.email)) return false;
  if (b.phone && !/^\+7\d{10}$/.test(b.phone)) return false; // при желании можно ослабить
  if (!(+b.guests >= 1 && +b.guests <= 5)) return false;
  if ((b.notes||'').length > 500) return false;
  return true;
}
// --- end helpers ---
...
<div class="w">
  ...
  <div class="card" style="margin-top:12px">
    ...
    <form onsubmit="return send(event)">
      ...
      <input id="hp" name="hp" autocomplete="off" tabindex="-1" aria-hidden="true" style="position:absolute;left:-9999px;opacity:0">
...
// __SECURE_RSVP_HANDLER__ (injected)
if (req.method==='POST' && req.url==='/api/rsvp'){
  let d=''; req.on('data', c => d += c);
  req.on('end', async () => {
    try{
      const ip = (req.headers['x-forwarded-for']?.split(',')[0] || req.socket.remoteAddress || '').trim();
      if (!isSameOrigin(req) || !rateOk(ip)) {
        res.writeHead(403, {'content-type':'application/json'}); return res.end('{"ok":false}');
      }
      const b = JSON.parse(d||'{}');
      if ((b.hp && String(b.hp).trim()!=='') || !validBody(b)) {
        res.writeHead(204); return res.end(); // тихий игнор ботов
      }
      await sendTG(b);
      res.writeHead(200, {'content-type':'application/json'}); res.end('{"ok":true}');
    }catch(e){
      res.writeHead(500, {'content-type':'application/json'}); res.end('{"ok":false}');
    }
  });
  return;
}
